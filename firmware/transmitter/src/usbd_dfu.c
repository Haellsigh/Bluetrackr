...
USBD_ClassTypeDef USBD_DFU = {USBD_DFU_Init,
                              USBD_DFU_DeInit,
                              USBD_DFU_Setup,
  ...
  if (hdfu->dev_state == DFU_STATE_DNLOAD_BUSY) {
    ...
    /* Regular Download Command */
      ...
      if (hdfu->wblock_num > 1U) {
        ...
        if (((USBD_DFU_MediaTypeDef*)pdev->pUserData)
                ->Write(hdfu->buffer.d8, (uint8_t*)addr, hdfu->wlength) != USBD_OK) {
          return USBD_FAIL;
        }
      }
    }
...
static uint8_t* USBD_DFU_GetUsrStringDesc(USBD_HandleTypeDef* pdev, uint8_t index, uint16_t* length)
{
  ...
  if (index <= (USBD_IDX_INTERFACE_STR + USBD_DFU_MAX_ITF_NUM)) {
    USBD_GetString((uint8_t*)((USBD_DFU_MediaTypeDef*)pdev->pUserData)->pStrDesc, USBD_StrDesc,
                   length);
    return USBD_StrDesc;
  }
...
static void DFU_Download(USBD_HandleTypeDef* pdev, USBD_SetupReqTypedef* req)
{
  ...
  if (req->wLength > 0U) {
    if ((hdfu->dev_state == DFU_STATE_IDLE) || (hdfu->dev_state == DFU_STATE_DNLOAD_IDLE)) {
      ...
      USBD_CtlPrepareRx(pdev, (uint8_t*)hdfu->buffer.d8, (uint16_t)hdfu->wlength);
    }
    /* Unsupported state */
    else {
      /* Call the error management function (command will be nacked */
  ...
  if (req->wLength > 0U) {
    if ((hdfu->dev_state == DFU_STATE_IDLE) || (hdfu->dev_state == DFU_STATE_UPLOAD_IDLE)) {
      ...
      if (hdfu->wblock_num == 0U) {
        ...
        addr = ((hdfu->wblock_num - 2U) * USBD_DFU_XFER_SIZE) +
               hdfu->data_ptr; /* Change is Accelerated*/

        /* Return the physical address where data are stored */
        phaddr = ((USBD_DFU_MediaTypeDef*)pdev->pUserData)
                     ->Read((uint8_t*)addr, hdfu->buffer.d8, hdfu->wlength);
    ...
    case DFU_STATE_DNLOAD_SYNC:
      if (hdfu->wlength != 0U) {
        ...
        if ((hdfu->wblock_num == 0U) && (hdfu->buffer.d8[0] == DFU_CMD_ERASE)) {
          ((USBD_DFU_MediaTypeDef*)pdev->pUserData)
              ->GetStatus(hdfu->data_ptr, DFU_MEDIA_ERASE, hdfu->dev_status);
        } else {
          ((USBD_DFU_MediaTypeDef*)pdev->pUserData)
              ->GetStatus(hdfu->data_ptr, DFU_MEDIA_PROGRAM, hdfu->dev_status);
        }
      } else /* (hdfu->wlength==0)*/
      {
