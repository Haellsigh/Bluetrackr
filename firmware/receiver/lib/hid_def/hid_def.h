...
#define HID_HEAD(tag, type, size) \
  (((tag) << 4) | _CAT2(HID_TYPE_, type) | (((size) < 4 ? (size) : 3) & 0x3))
...
#define HID_DATA4(x) \
  , (((x) >> 16) & 0xff), (((x) >> 24) & 0xff), (((x) >> 0) & 0xff), (((x) >> 8) & 0xff)
...
#define HID_USAGE_BUTTON(n, x)                                                               \
  n, (0x00 + (_ASSERT(1 <= (n) && (n) <= 2 && 1 <= (x) && (x) <= ((n) == 1 ? 0xff : 0xffff)) \
                  ? (x)                                                                      \
                  : (0))) /* x: [1..255] (n:1) or [1..65535] (n:2) */

/* Ordinal Usages */
#define HID_USAGE_INSTANCE(n, x)                                                             \
  n, (0x00 + (_ASSERT(1 <= (n) && (n) <= 2 && 1 <= (x) && (x) <= ((n) == 1 ? 0xff : 0xffff)) \
                  ? (x)                                                                      \
                  : (0))) /* x: [1..255] (n:1) or [1..65535] (n:2) */
...
#define HID_USAGE_OPTIONAL_MFG_FUNCTION(n) \
  1, (0x09 + (_ASSERT(1 <= (n) && (n) <= 5) ? (n) : (0))) /* n: [1..5] */
...
#define HID_USAGE_CAPACITY_GRANULARITY(n) \
  1, (0x8c + (_ASSERT(1 <= (n) && (n) <= 2) ? (n) : (n))) /* n: [1..2] */
...
#define HID_PAGED_USAGE(p, u) \
  HID_ELEM(0x0, LOCAL, 4, (_NTH1(_CAT2(HID_USAGE_PAGE_, p)) << 16) | _NTH1(_CAT2(HID_USAGE_, u)))
